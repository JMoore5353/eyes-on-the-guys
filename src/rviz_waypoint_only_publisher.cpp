...
#include "rviz_waypoint_only_publisher.hpp"

namespace rosplane_gcs
{

RvizWaypointOnlyPublisher::RvizWaypointOnlyPublisher()
    : Node("rviz_waypoint_only_publisher")
{
  declare_parameters();
  ...
  waypoint_sub_ = this->create_subscription<rosplane_msgs::msg::Waypoint>(
    "waypoint_path", 10,
    std::bind(&RvizWaypointOnlyPublisher::new_wp_callback, this, std::placeholders::_1));

  num_wps_ = 0;
}

void RvizWaypointOnlyPublisher::declare_parameters()
{
  this->declare_parameter("waypoint_scale", 1.5);
  this->declare_parameter("text_scale", 5.0);
}

void RvizWaypointOnlyPublisher::new_wp_callback(const rosplane_msgs::msg::Waypoint & wp)
{
  if (wp.clear_wp_list) {
...
void RvizWaypointOnlyPublisher::publish_markers_to_clear_waypoints()
{
...
RvizWaypointOnlyPublisher::create_new_waypoint_marker(const rosplane_msgs::msg::Waypoint & wp)
{
  visualization_msgs::msg::Marker new_marker;
  new_marker.header.stamp = this->get_clock()->now();
  new_marker.header.frame_id = "NED";
...
void RvizWaypointOnlyPublisher::update_waypoint_line_list(const rosplane_msgs::msg::Waypoint & wp)
{
...
visualization_msgs::msg::Marker
RvizWaypointOnlyPublisher::create_new_waypoint_text_marker(const rosplane_msgs::msg::Waypoint & wp)
{
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<rosplane_gcs::RvizWaypointOnlyPublisher>();
